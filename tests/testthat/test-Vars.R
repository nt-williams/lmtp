test_that("create proper node lists, t > 1", {
  verify_output(test_path("test-Vars.txt"), {
    ...
    # k = Inf
    foo <- LmtpVars$new(bs, nodes, a, NULL, "y", "binomial", 4)
    ...
    # k = 0
    foo <- LmtpVars$new(bs, nodes, a, NULL, "y", "binomial", 4, k = 0)
    lapply(1:4, function(t) {
      list(trt = foo$history("A", t),
           outcome = foo$history("L", t + 1))
    })
    ...
    # k = 1
    foo <- LmtpVars$new(bs, nodes, a, NULL, "y", "binomial", 4, k = 1)
    lapply(1:4, function(t) {
      list(trt = foo$history("A", t),
           outcome = foo$history("L", t + 1))
    })
    ...
    # k = 2
    foo <- LmtpVars$new(bs, nodes, a, NULL, "y", "binomial", 4, k = 2)
    lapply(1:4, function(t) {
      list(trt = foo$history("A", t),
           outcome = foo$history("L", t + 1))
    })
    ...
    # NULL time-varying
    foo <- LmtpVars$new(bs, NULL, a, NULL, "y", "binomial", 4)
    lapply(1:4, function(t) {
      list(trt = foo$history("A", t),
           outcome = foo$history("L", t + 1))
    })
    ...
    foo <- LmtpVars$new(bs, NULL, "A", NULL, paste0("Y_", 1:4), "survival", 4)
    lapply(1:4, function(t) {
      list(trt = foo$history("A", t),
           outcome = foo$history("L", t + 1))
    })
...
test_that("rename variable correctly", {
  ...
  foo <- LmtpVars$new(bs, nodes, a, NULL, "y", "binomial", 2)
