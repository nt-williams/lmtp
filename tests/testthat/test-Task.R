...
test_that("task creation with survival outcome", {
  ...
  expect_equal(head(task$at_risk_N(sim_point_surv, 0)), rep(TRUE, 6))
  expect_equal(head(task$at_risk_D(sim_point_surv, 0)), rep(TRUE, 6))
  expect_equal(head(task$at_risk_N(sim_point_surv, 5)), c(F, F, F, T, F, F))
  expect_equal(head(task$at_risk_D(sim_point_surv, 5)), rep(TRUE, 6))
})
