...
test_that("No uncensored missing data", {
  ...
  expect_error(
    sw(lmtp_sub(sim_cens, A, "Y", time_vary = L, cens = cens)),
...
test_that("Reserved variables", {
  ...
  expect_error(
    sw(lmtp_sub(sim_cens, A, "tmp_lmtp_stack_indicator", time_vary = L, cens = cens)),
...
test_that("Incorrect folds", {
  ...
  expect_error(
    sw(lmtp_sub(sim_cens, A, "Y", time_vary = L, cens = cens, folds = 0)),
...
test_that("Issues with 'outcome_type' being or not being survival", {
  ...
  expect_error(
    sw(lmtp_sub(sim_cens, A, "Y", time_vary = L, cens = cens, outcome_type = "survival")),
  ...
  expect_error(
    sw(lmtp_ipw(sim_point_surv, A, Y, W, cens = cens, shift = static_binary_on)),
...
test_that("Contrast assertions", {
  ...
  fit <- sw(lmtp_sub(sim_cens, A, "Y", time_vary = L, cens = cens, folds = 1))
  expect_error(
