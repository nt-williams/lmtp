...
sub <- lmtp_sub(tmp, list("A"), "Y", baseline = c("W1", "W2"), shift = static_binary_on, folds = 1)
ipw <- lmtp_ipw(tmp, list("A"), "Y", baseline = c("W1", "W2"), shift = static_binary_on, folds = 1)
tmle <- lmtp_tmle(tmp, list("A"), "Y", baseline = c("W1", "W2"), shift = static_binary_on, folds = 1)
sdr <- lmtp_sdr(tmp, list("A"), "Y", baseline = c("W1", "W2"), shift = static_binary_on, folds = 1)
