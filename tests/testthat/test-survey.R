...
sub <- sw(lmtp_sub(tmp, list("A"), "Y", baseline = c("W1", "W2"), shift = static_binary_on,
                weights = wts, folds = 2))

ipw <- sw(lmtp_ipw(tmp, list("A"), "Y", baseline = c("W1", "W2"), shift = static_binary_on,
                weights = wts, folds = 2))

tmle <- sw(lmtp_tmle(tmp, list("A"), "Y", baseline = c("W1", "W2"), shift = static_binary_on,
                  weights = wts, folds = 2))

sdr <- sw(lmtp_sdr(tmp, list("A"), "Y", baseline = c("W1", "W2"), shift = static_binary_on,
                weights = wts, folds = 2))
