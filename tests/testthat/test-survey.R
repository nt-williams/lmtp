...
Y.1 <- rbinom(n, size = 1, prob = plogis(-1 + 1 + 1.5 * W1 + 0.3 * W2))
Y.0 <- rbinom(n, size = 1, prob = plogis(-1 + 0 + 1.5 * W1 + 0.3 * W2))

Y <- Y.1 * A + Y.0 * (1 - A)
tmp <- data.frame(W1, W2, A, Y, Y.1, Y.0)
truth <- mean(tmp$Y.1)

prob_S <- plogis(2 * W1 - 1)
S <- rbinom(n, 1, prob_S)
tmp <- tmp[S == 1, ]
wts <- 1 / prob_S[S == 1]
...
tmle2 <- sw(ltmle(tmp, "A", "Y", baseline = c("W1", "W2"), weights = wts, folds = 2))
