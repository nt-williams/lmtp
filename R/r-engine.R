...
estimate_r <- function(data, A, C, shift, tau,
  ...
  lapply(1:tau, function(t) {
    ...
    r$natural[!pid, t] <<- 0
    r$shifted[!pid, t] <<- 0
    r$natural[pid, t] <<- rat[d$shift_indicator == 0] * C[pid, t]
    r$shifted[pid, t] <<- rat[d$shift_indicator == 1] * C[pid, t]
  })
...
estimate_c <- function(data, C, Y, tau, node_list, learner_stack) {
  ...
  # setup
  cens <- matrix(nrow = nrow(data), ncol = tau)

  if (isFALSE(check_censoring(data, C, Y))) {
    lapply(1:tau, function(t) {
      cens[, t] <<- rep(1, nrow(data))
    })
    return(cens)
  }
