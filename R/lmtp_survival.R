...
lmtp_survival <- function(data, trt, outcomes, baseline = NULL, time_vary = NULL,
  ...
  is_new_time_vary <- is.list(time_vary) && !is.null(time_vary) && !is.null(names(time_vary))
  if (!is_new_time_vary && length(time_vary) == 1) args$time_vary <- time_vary
  ...
  for (time in seq_len(time_horizon)) {
    ...
    if (is_new_time_vary) {
      args$time_vary <- list(
        trt     = time_vary$trt[seq_len(time)],
        cens    = time_vary$cens[seq_len(time)],
        outcome = time_vary$outcome[seq_len(time)]
      )
    } else if (length(time_vary) > 1) {
