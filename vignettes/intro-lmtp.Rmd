...
In contrast, we may interested in the effect of a modified treatment policy where exposure is only increased by 7 units at all time points for observations who are below some threshold, such as 50:
...
```{r, eval = F}
# using a shift function in an estimator
lmtp_sdr(...)
```
...
```{r}
# set exposure to 1 for all observations
shift <- function(x) 1

# set exposure to 0 for all observations
...
In certain situtations, the user may be interested in the population mean outcome under no intervention. In the presence of censoring, this can be estimated by setting `trt = NULL` and by indicating censoring nodes. 
