...
How these nodes are specified depends on the specific data generating mechanism and should be pre-specified based on a conceptual model (i.e, a DAG). How these nodes are used by `lmtp` estimators is specified by what we call a node list. The analyst doesn't explicitly create the node list them-self, instead the analyst supplies the ingredients and instructions on how to combine all the different nodes; this is done through the `k` parameter. 
...
Notice that the time-varying covariates are specified using a list of vectors, allowing the user to specify multiple time-varying covariates at each time-point of observation. The length of this list should be the same length as the treatment vector, `a` in this example (the one exception to this rule is in [Survival analysis]). 
