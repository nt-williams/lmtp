...
Computation time can quickly increase in with many time-points, a large Super Learner library, and large datasets. To help, **lmtp** provides support for parallel processing using **future** [@future]. The simplest way to use estimators in parallel is to run `plan(multiprocess)`. We recommend consulting the **future** [documentation](https://cran.r-project.org/web/packages/future/future.pdf) for more information.
